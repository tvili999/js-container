...
export default class Runners {
  ...
  dependency(name: RunnerKey, dependency: Dependency) {
    let dependsOn = wrapInArray(dependency.dependsOn);
    let dependentBy: RunnerKey[] = wrapInArray(dependency.dependentBy);
  ...
  async run(name: RunnerKey) {
    ...
    if (!runner) throw `Runner '${name.toString()}' does not exist`;
    if (this.ran.includes(name)) return;

    if (this.runQueue.includes(name))
      throw `Circular runner dependency: ${this.runQueue.join(
        " -> ",
      )} -> ${name.toString()}`;
    this.runQueue.push(name);

    const displayName =
      typeof name === "symbol" ? (name as Symbol).description : name;
  ...
  async runAll() {
    for (const name of this.runners.keys()) await this.run(name);
  }
}
